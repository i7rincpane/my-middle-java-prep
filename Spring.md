...
Spring — это огромный конструктор, где ты сам решаешь, какие детали нужны и как их соединить. Spring Boot — это «собраная модель» с инструкцией, которая заводится одной кнопкой.
...
### Spring Boot (Надстройка над Spring)
...
## <a id="transactional-p">Transactional</a>
...
```
### Почему methodA вызывающий methodB ломается?
...
### Как получить прокси внутри самого бина?
...
## <a id="security-p">Spring Security</a>
Spring Security — это сложная цепочка фильтров (Security Filter Chain). Независимо от способа (сессия или JWT), цель одна: идентифицировать пользователя и положить его данные в **SecurityContextHolder** (ThreadLocal) и решить, можно ли пользователю войти в конкретный метод (**Authorization**).
...
### Authentication: Как сервер узнает пользователя?
...
### Защита CSRF (Cross-Site Request Forgery)
...
### Как это выглядит на низком уровне (Spring Data JPA)
...
### Примеры SQL, которые генерирует Hibernate
...
### Проблема производительности (Deep Pagination)
...
## <a id="filtering-p">Динамическая фильтрация</a>
...
### Архитектура EAV (Entity-Attribute-Value)
