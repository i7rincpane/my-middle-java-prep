...
* ### [Структура памяти JVM](#jvm-memory-structure-p)
* ### [Сборка мусора (Garbage Collection)](#garbage-collection-p)
* ### [Виды Garbage Collectors (GC)](#gc-type-p)
* ### [Утечки памяти (Memory Leaks)](#memory-leaks-p)
* ### [Типы ссылок в Java](#leaks-type-p)
* ### [Диагностика и профилирование](#profiling-p)

## <a id="jvm-memory-structure-p">Структура памяти JVM</a>
...
Общий объем оперативной памяти, которую занимает весь процесс Java (Native Memory), включает в себя: **Heap + Metaspace + Code Cache + Stacks потоков**. Поэтому потребление памяти процессом всегда выше, чем указанный `-Xmx`.
...
* **Суть экономии**: Поскольку строка неизменяема, Java безопасно кэширует её в **String Pool**. Разные переменные с одинаковым текстом ссылаются на один объект в памяти вместо создания дубликатов.
* **Что если бы строки изменялись**: Присвоение `String b = a` требовало бы глубокого копирования и создания нового объекта для каждой переменной. Иначе изменение переменной `a` незаметно испортило бы данные в переменной `b`.
* **Проблема модификации**: Обратная сторона иммутабельности — любое изменение (конкатенация, замена символа) порождает новый объект в куче (Heap), загрязняя память временным мусором.
* **Как это оптимизируется**:
  * **На этапе компиляции**: Сложение строк-литералов (например, `"a" + "b" + "c"`) компилятор сам склеивает в одну готовую строку `"abc"`.
  * **В рантайме**: Для частых изменений в циклах разработчик вручную использует `StringBuilder`. Он работает на базе одного изменяемого массива байтов и не плодит новые объекты при каждой операции.
...
## <a id="garbage-collection-p">Сборка мусора (Garbage Collection)</a>

JVM не ищет мусор. Она ищет **живых**, а всё остальное считает пустотой. Работа GC основана на **гипотезе о поколениях**: большинство объектов живут очень недолго. Поэтому вместо одной тяжелой уборки во всей памяти, JVM делает множество быстрых «микро-уборок» в молодой области (Young Gen).
...
## <a id="gc-type-p">Виды Garbage Collectors (GC)</a>

Эволюция сборщиков мусора — это борьба за сокращение пауз STW. Мы переходим от простой остановки всего мира (Serial/Parallel) к фоновой пометке мусора (CMS/G1) и, наконец, к фоновому перемещению объектов (ZGC), платя за это ресурсами процессора и памятью.
...
### Epsilon GC (No-Op Garbage Collector)  — `-XX:+UseEpsilonGC`
...
## <a id="memory-leaks-p">Утечки памяти (Memory Leaks)</a>

Утечка памяти в Java - это не "исчезновение" памяти, а удержание ненужных объектов. Основная коварность утечек заключается в старении мусора: если объект не успел удалиться в Young Gen, он уходит в Old Gen, где Full GC будет пытаться достать его ценой огромных пауз.
...
## <a id="leaks-type-p">Типы ссылок в Java</a>
...
## <a id="profiling-p">Диагностика и профилирование</a> 

Профилировщик — это инструмент, который анализирует **динамику** работы кода (время, частоту вызовов, скорость заполнения памяти) и позволяют увидеть, на что тратится процессорное время и чем забит Хип на реальном работающем приложении.
