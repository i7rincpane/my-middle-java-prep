...
### Шпаргалка: Паттерн Singleton с Double-Checked Locking (DCL)

#### Реализация паттерна

```java
public class Singleton {
// Ключевое слово volatile критически важно!
private static volatile Singleton instance;

    private Singleton() {
        // Приватный конструктор защищает от создания через new
    }

    public static Singleton getInstance() {
        // Первая проверка (без синхронизации для скорости)
        if (instance == null) { 
...
1. **Первая проверка** (`if (instance == null)`): Поток проверяет, создан ли объект. Если да, он сразу его возвращает. Это спасает производительность, так как потоки не стоят в очереди к `synchronized` при повторных вызовах.
2. **Блок** `synchronized (Singleton.class)`: Если объекта нет, поток пытается захватить монитор класса. Статический контекст обязывает нас блокироваться именно по объекту `Singleton.class`, так как экземпляра класса еще нет в памяти.
3. **Вторая проверка** (`if (instance == null)`): Защищает от ситуации, когда два потока одновременно прошли первую проверку. Первый поток захватил монитор и создал объект. Когда он отпустит монитор, второй поток зайдет в блок и без этой проверки создал бы дубликат.

#### Роль ключевого слова volatile

Без `volatile` код сломается в многопоточной среде из-за оптимизации процессора (**Instruction Reordering**). Создание объекта `new Singleton()` состоит из трех шагов:
1. Выделение памяти.
2. Вызов конструктора (инициализация полей).
3. Запись ссылки на память в переменную `instance`.

Процессор может поменять местами шаги 2 и 3. Тогда переменная `instance` перестанет быть `null` **до того**, как отработает конструктор. Другой поток на первой проверке увидит, что ссылка не `null`, заберет этот «сырой» объект и упадет с ошибкой. `volatile` принудительно запрещает перестановку этих инструкций.
