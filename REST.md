...
### 4. Идемпотентность: Борьба с дубликатами при разрыве 

Идемпотентность — это свойство метода при повторном вызове давать тот же результат, что и при первом, 
Проще говоря: «сколько бы раз ты ни нажал на кнопку, результат будет как от одного нажатия».
